...
import { getAllForms, getFormById, updateForm } from "../services/form.service";
...
const Forms = () => {
  ...
  const handleDrop = useCallback(async (e, dropIdx) => {
    ...
    try {
      await Promise.all(
        updated.map((f) =>
          updateForm(f._id, { order: f.order })
        )
      );
    } catch (err) {
  ...
  return (
    <>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-[#014b62] text-white">
                    ...
                    <th className="py-3 px-6 font-semibold w-36 text-center">Status</th>
                  </tr>
                </thead>
                <tbody>
                  {forms.map((form, index) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-4 font-bold text-[#014b62]">{form.order}</td>
                      <td className="py-4 px-4 font-bold text-[#014b62]">{form.title}</td>
                      <td className="py-4 px-4 font-semibold text-[#014b62]">{form.description}</td>
      ...
      {/* Loading overlay */}
      {loadingForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: "rgba(100,116,125,0.4)" }}>
